    ...
    try {
        ...
        try {
            ...
            for (const i of items) {
                ...
                if(purchased_qty < 1) throw new Error(`need more than 0 to increase Item qty with ID ${i[0]} not found.`);
                
                if (itemInfo.qty - purchased_qty < 0) throw new Error(`quantity demand cannot be met by the quantity of goods available`);
            ...
            await t.commit();
            res.status(200).json({ message: message + "transaction created successfully." });
